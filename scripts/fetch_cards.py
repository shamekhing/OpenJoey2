...
what DuelScreen loads). The JPEG is re-encoded as a real PNG with Pillow.
...
Requires Pillow (pip install pillow) for the card-back PNG re-encode.
...
def download_card_back(do_download):
    ...
    try:
        ...
        # PIL is mandatory: a real PNG on disk (never raw JPEG bytes in a .png
        # name — tools trusting the extension would misread it).
        from PIL import Image
        import io
        Image.open(io.BytesIO(data)).save(str(CARD_BACK), "PNG")
        print("ok  (%dKB)" % (CARD_BACK.stat().st_size // 1024))
    except ImportError:
        print("FAILED: PIL/Pillow required (pip install pillow)")
    except Exception as e:
