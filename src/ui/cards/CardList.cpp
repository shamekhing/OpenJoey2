...
void CardList::DrawItem(const openjoey::cards::Card& card, CardImageCache& cache, int x, int y, int w, bool selected, int copies, int maxCopies) {
...
void CardList::Draw(const std::vector<const openjoey::cards::Card*>& cards, CardImageCache& cache, int x, int y, int w, int h, int cursor, bool focused, int maxCopies, std::function<int(uint32_t)> countFn) {
    const int itemH = CARD_ITEM_HEIGHT;
...
Color CardList::cardTypeColor(const openjoey::cards::Card& c) { return c.isMonster() ? MAROON : (c.isSpell() ? GREEN : PINK); }
