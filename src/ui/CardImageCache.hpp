...
// Call Request() each frame for cards on screen; call PollAndLoad() each frame
...
public:
  ...
  // Call from main thread — returns texture if ready, nullptr otherwise.
  // Automatically queues a download if not on disk yet.
  const Texture2D *Get(const openjoey::Card &card) {
    ...
    // Not in GPU yet — request download if needed
  ...
  // Call once per frame from main thread to upload completed downloads.
  void PollAndLoad() {
...
private:
  ...
  void workerLoop() {
    while (true) {
      Job job;
      {
        ...
        cv_.wait(lk, [this] { return stop_ || !jobQueue_.empty(); });
        if (stop_)
          return;
      ...
      bool ok = curlDownload("https://images.ygoprodeck.com/images/cards/" +
                                 std::to_string(job.id) + ".jpg",
                             job.dest);
      if (!ok)
        ok = curlDownload("https://images.ygoprodeck.com/images/cards_small/" +
                              std::to_string(job.id) + ".jpg",
                          job.dest);
  ...
  static bool curlDownload(const std::string &url,
    ...
    std::string cmd =
        "curl -sSL --max-time 20 -o '" + tmp + "' '" + url + "' 2>/dev/null";
  ...
  std::filesystem::path imgDir_ =
      std::filesystem::current_path() / "data" / "images";
