...
TEST_CASE("Trap set this turn cannot be activated; next turn it can (p.31)",
    ...
    ActionArgs same;
    same.source = &trap;
    REQUIRE(action::ActivateEffect(d, trapFx, 0, same).msg.find("turn it was Set") != std::string::npos);
    ...
    ActionArgs next;
    next.source = &trap;
    REQUIRE(action::ActivateEffect(d, trapFx, 0, next).msg.find("Chain Link") != std::string::npos);
...
TEST_CASE("PassResponse: disabled flag no-ops; ChainWaiting stays false (p.45 mode off)",
    ...
    CHECK(action::PassResponse(d, 0).msg.find("disabled") != std::string::npos);
    ...
    CHECK(action::ActivateEffect(d, dmg, 0).msg.find("Chain Link") != std::string::npos);
    CHECK(action::PassResponse(d, 0).msg.find("disabled") != std::string::npos);
...
TEST_CASE("Chain Spell Speed rule + LP effects mutate Life Points (p.41)",
    ...
    REQUIRE(action::ActivateEffect(d, ActionSpec{ActionId::LP_Damage, EffectType::Ignition, 1, 300}, 0).msg.find("Chain Link 1") != std::string::npos);
    // SS1 can never be Chain Link 2+.
    REQUIRE(action::ActivateEffect(d, ActionSpec{ActionId::LP_Gain, EffectType::Ignition, 1}, 1).msg.find("illegal chain") != std::string::npos);
    // A response must be at least as fast as the link it responds to.
    REQUIRE(action::ActivateEffect(d, ActionSpec{ActionId::LP_Gain, EffectType::Ignition, 2, 500}, 1).msg.find("Chain Link 2") != std::string::npos);
    REQUIRE(action::ActivateEffect(d, ActionSpec{ActionId::LP_Gain, EffectType::Ignition, 1}, 0).msg.find("illegal chain") != std::string::npos);
    ...
    std::string log = action::ResolveChain(d).msg;
    CHECK(d.chain.links.empty());
    ...
    REQUIRE(action::ActivateEffect(d, ActionSpec{ActionId::Cost_PayLP, EffectType::Ignition, 1, 800}, 0).msg.find("Chain Link 1") != std::string::npos);
    action::ResolveChain(d);
...
TEST_CASE("Chain negation: a counter blanks the link it responds to (p.44)",
    ...
    CHECK(action::ActivateEffect(d, ActionSpec{ActionId::Move_Draw, EffectType::Ignition, 2, 2}, 0).msg.find("Chain Link 1") != std::string::npos);
    // P1 counters with a negation (SS 3) — Chain Link 2.
    CHECK(action::ActivateEffect(d, ActionSpec{ActionId::NegateActivation, EffectType::Quick, 3}, 1).msg.find("Chain Link 2") != std::string::npos);
    // SS 1 can never be Chain Link 2+ (p.41).
    CHECK(action::ActivateEffect(d, ActionSpec{ActionId::Move_MillToGY, EffectType::Ignition, 1}, 0).msg.find("illegal chain") != std::string::npos);

    const std::string log = action::ResolveChain(d).msg;
    ...
    CHECK(action::ActivateEffect(d, ActionSpec{ActionId::NegateEffect, EffectType::Quick, 3}, 0).msg.find("nothing to negate") != std::string::npos);

    // Cost_PayLP (Solemn-style) really mutates LP at resolution.
    CHECK(action::ActivateEffect(d, ActionSpec{ActionId::Cost_PayLP, EffectType::Ignition, 3, 1000}, 0).msg.find("Chain Link 1") != std::string::npos);
    action::ResolveChain(d);
...
TEST_CASE("Summon_Flip link triggers the flipped card's effect (Man-Eater)",
    ...
    ActionArgs fa;
    fa.target = &flipper;
    CHECK(action::ActivateEffect(d, ActionSpec{ActionId::Summon_Flip}, 0, fa).msg.find("Chain Link 1") != std::string::npos);
    const std::string log = action::ResolveChain(d).msg;
