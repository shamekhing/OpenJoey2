...
inline ActionResult SummonNormal(Duel &d, Card *c) {
    if (!c || !c->isMonster())
        return ActionResult::Fail("not a monster.");
    if (!CanNormalSummon(d))
        return ActionResult::Fail(
            "no normal summon available (once per turn, Main Phase only).");
    if (TributesRequired(c) > 0)
        return ActionResult::Fail("level " + std::to_string(c->level) +
                                  " requires tribute(s) — use tributeSummon.");
    if (!SummonToMMZ(d.field, c, d.turnPlayer, /*faceDown=*/false))
        return ActionResult::Fail("summon failed (no free monster zone).");
    d.turnState.normalSummonUsed = true;
    c->state.placedThisTurn = true;
    return ActionResult::Ok("player " + std::to_string(d.turnPlayer) +
                            " normal summons " + c->name + " (ATK).");
}
...
inline ActionResult SummonSet(Duel &d, Card *c) {
    ...
    if (!CanNormalSummon(d))
        return ActionResult::Fail(
            "no normal set available (once per turn, Main Phase only).");
    if (TributesRequired(c) > 0)
        return ActionResult::Fail("level " + std::to_string(c->level) +
                                  " requires tribute(s) — use tributeSummon.");
    if (!SummonToMMZ(d.field, c, d.turnPlayer, /*faceDown=*/true))
        return ActionResult::Fail("set failed (no free monster zone).");
    d.turnState.normalSummonUsed = true;
...
inline ActionResult SummonTribute(Duel &d, Card *c,
    ...
    if ((int)tributes.size() != need)
        return ActionResult::Fail("level " + std::to_string(c->level) +
                                  " requires exactly " + std::to_string(need) +
                                  " tribute(s).");
    for (Card *t : tributes) {
        zone::Zone_Monster *tz = d.field.monsterZoneOf(t);
        if (!tz || t->state.controller != d.turnPlayer || t == c)
            return ActionResult::Fail(
                "invalid tribute: must be your own monster on the field.");
    }
    for (Card *t : tributes)
...
inline ActionResult FlipSummon(Duel &d, Card *c) {
    if (d.result != DuelResult::Ongoing ||
        ...
        return ActionResult::Fail("Flip Summon needs a Main Phase.");
    zone::Zone_Monster *mz = d.field.monsterZoneOf(c);
    if (!mz || c->state.controller != d.turnPlayer)
        return ActionResult::Fail("not your monster in a monster zone.");
    if (mz->isVisible())
        return ActionResult::Fail("monster is not face-down.");
    if (c->state.setThisTurn)
        return ActionResult::Fail("cannot Flip Summon the turn it was Set.");
...
inline ActionResult ChangePosition(Duel &d, Card *c) {
    if (d.result != DuelResult::Ongoing ||
        (d.turn.phase != Phase::Main1 && d.turn.phase != Phase::Main2))
        return ActionResult::Fail("position change needs a Main Phase.");
    zone::Zone_Monster *mz = d.field.monsterZoneOf(c);
    if (!mz || c->state.controller != d.turnPlayer)
        return ActionResult::Fail("not your monster in a monster zone.");
    if (!mz->isVisible())
    ...
    if (c->state.placedThisTurn || c->state.setThisTurn ||
        ...
        return ActionResult::Fail("cannot change position the turn it arrived.");
    if (d.turnState.attacked.count(c))
        return ActionResult::Fail(
            "cannot change position after attacking this turn (p.36).");
    if (d.turnState.positionChanged.count(c))
        return ActionResult::Fail("position already changed this turn.");
    zone::Orientation to = (mz->position() == zone::Orientation::Vertical)
                               ? zone::Orientation::Horizontal
                               : zone::Orientation::Vertical;
    if (!mz->changeOrientation(to))
        return ActionResult::Fail("position change failed.");
    d.turnState.positionChanged.insert(c);
    return ActionResult::Ok(
        c->name + std::string(to == zone::Orientation::Horizontal
                                  ? " switches to DEF."
                                  : " switches to ATK."));
}
...
inline ActionResult SpecialSummon(Duel &d, Card *c, SpecialPose pose) {
    ...
    if (!d.canAct())
        return ActionResult::Fail("special summon needs an action window.");
    if (!c) return ActionResult::Fail("no card.");
    auto [z, p] = d.field.findCard(c);
    if (!z) return ActionResult::Fail("card is not in any zone.");
    if (c->state.controller != d.turnPlayer)
        return ActionResult::Fail("not your card.");
    if (!SummonFromZone(d.field, c, d.turnPlayer, /*toEMZ=*/false,
                        ...
                        : pose == SpecialPose::DefUp ? SummonPose::DefUp
                                                     : SummonPose::DefDown))
        return ActionResult::Fail("special summon failed (no free monster zone).");
    return ActionResult::Ok(
        "player " + std::to_string(d.turnPlayer) + " special summons " +
        c->name +
...
inline ActionResult SpecialSummon(Duel &d, Card *c, bool faceDown = false) {
...
inline ActionResult FusionSummon(Duel &d, Card *extra,
                                 const std::vector<Card *> &materials) {
    if (d.result != DuelResult::Ongoing ||
        (d.turn.phase != Phase::Main1 && d.turn.phase != Phase::Main2))
        return ActionResult::Fail("Fusion Summon needs a Main Phase.");
    if (!extra) return ActionResult::Fail("no fusion monster.");
    if (materials.size() < 2)
        return ActionResult::Fail("a Fusion Summon needs at least 2 material(s).");
    auto [z, p] = d.field.findCard(extra);
    if (!z || z->type() != zone::ZoneType::ExtraDeck)
        return ActionResult::Fail(
            "the fusion monster must be in its owner's Extra Deck.");
    for (Card *m : materials) {
        if (!m || m->state.controller != d.turnPlayer)
            return ActionResult::Fail("fusion materials must be your own monsters.");
        ...
        if (!z || (z->type() != zone::ZoneType::Monster &&
            ...
            return ActionResult::Fail(
                "fusion materials must be monsters on the field or in your hand.");
    }
    if (!PlaceFusion(d.field, extra))
        return ActionResult::Fail("fusion summon failed (no free Extra Monster Zone).");
    MoveMaterialsToGY(d.field, materials);
    return ActionResult::Ok("player " + std::to_string(d.turnPlayer) +
                            " Fusion Summons " + extra->name + " (" +
                            std::to_string(materials.size()) +
                            " material(s) -> Graveyard).");
}

// Ritual Summon (p.21): hand -> MMZ; tribute levels >= ritual level.
inline ActionResult RitualSummon(Duel &d, Card *monster,
                                 const std::vector<Card *> &tributes) {
    if (d.result != DuelResult::Ongoing ||
        (d.turn.phase != Phase::Main1 && d.turn.phase != Phase::Main2))
        return ActionResult::Fail("Ritual Summon needs a Main Phase.");
    ...
    if (!z || z->type() != zone::ZoneType::Hand ||
        ...
        return ActionResult::Fail("the ritual monster must be in your hand.");
    if (tributes.empty())
        return ActionResult::Fail("a Ritual Summon needs at least 1 tribute.");
    int total = 0;
    for (Card *t : tributes) {
        if (!t || !t->isMonster())
            return ActionResult::Fail("tributes must be monsters.");
        ...
        if (!tz || t->state.controller != d.turnPlayer ||
            ...
            return ActionResult::Fail(
                "tributes must be your own monsters on the field or in hand.");
        total += t->level;
    }
    if (total < monster->level)
        return ActionResult::Fail("tribute levels (" + std::to_string(total) +
                                  ") are below level " +
                                  std::to_string(monster->level) + ".");
    if (!SummonToMMZ(d.field, monster, d.turnPlayer, /*faceDown=*/false))
        return ActionResult::Fail("ritual summon failed (no free monster zone).");
    MoveMaterialsToGY(d.field, tributes);
    return ActionResult::Ok("player " + std::to_string(d.turnPlayer) +
                            " Ritual Summons " + monster->name + " (" +
                            std::to_string(tributes.size()) +
                            " tribute(s) -> Graveyard).");
}
