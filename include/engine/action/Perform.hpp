...
inline std::string Perform(Duel &d, ActionId id, const ActionArgs &args = {}) {
  ...
  switch (id) {
  case ActionId::Cost_Tribute: case ActionId::Cost_Discard:
  case ActionId::Cost_BanishCost: case ActionId::Move_Draw:
  case ActionId::Move_MillToGY: case ActionId::Move_DiscardToGY:
  case ActionId::Move_DestroyToGY: case ActionId::Move_SendToGY:
  case ActionId::Move_Banish: case ActionId::Move_ReturnHand:
  case ActionId::Move_ReturnDeck: case ActionId::Summon_Normal:
  case ActionId::Summon_Set:
  case ActionId::Summon_Special: case ActionId::Summon_Fusion:
  ...
  case ActionId::NegateEffect:
    return ActivateEffect(d, ActionSpec{id}, me, args);
  case ActionId::DrawCard: case ActionId::Draw: case ActionId::DrawFirstCard:
    return DrawForTurn(d);
  case ActionId::SkipDraw: return "draw skipped.";
  ...
  case ActionId::SummonOrSetMonster: case ActionId::NormalSummon:
  case ActionId::PlayMonsterFaceUpAttack:
    return SummonNormal(d, args.target);
  case ActionId::NormalSet: case ActionId::PlayMonsterFaceDownDefense:
    return SummonSet(d, args.target);
  case ActionId::TributeSummon: case ActionId::SendTributeToGraveyard:
  ...
  case ActionId::FlipSummon: case ActionId::FlipToFaceUpAttack:
    return FlipSummon(d, args.target);
  case ActionId::ActivateFlipEffectTrigger: return ResolveChain(d);
  case ActionId::SpecialSummon: case ActionId::SpecialSummonFromHand:
  case ActionId::SpecialSummonFromGraveyard: case ActionId::SpecialSummonFromBanished:
  case ActionId::SpecialSummonFromExtraDeck: case ActionId::SpecialSummonFaceUp:
  ...
  case ActionId::ChangeMonsterBattlePosition: case ActionId::ChangeToAttackPosition:
  case ActionId::ChangeToDefensePosition:
    return ChangePosition(d, args.target);
  case ActionId::ActivateCardEffect: case ActionId::ActivateSpellEffect:
  case ActionId::ActivateTrapEffect: case ActionId::ActivateMonsterEffect:
    return ActivateEffect(d, args.spec, me, args);
  case ActionId::SetSpellCard: case ActionId::SetTrapCard:
    return "set: seat the card via the spell/trap zone.";
  case ActionId::DeclareAttack: case ActionId::SelectMonsterToAttackWith:
    return DeclareAttack(d, args.target,
                         args.materials.empty() ? nullptr : args.materials.front());
  case ActionId::SelectAttackTarget: case ActionId::AttackMonster:
  ...
  case ActionId::ConfirmAttack: case ActionId::ConfirmAttackResolution:
    return ResolveDamage(d);
  case ActionId::CanChooseNotToAttack: return "you may choose not to attack.";
  case ActionId::CancelAttack:
    CancelAttack(d); return "attack cancelled.";
  case ActionId::ReturnToMainPhase2: return ToMain2S(d);
  case ActionId::RespondWithEffect: case ActionId::AddToChain:
    return ActivateEffect(d, args.spec, me, args);
  case ActionId::PassChain: case ActionId::PassPriority:
    d.chain.step = protocol::ChainStep::Responding; return "priority passed.";
  case ActionId::ResolveChain: return ResolveChain(d);
  case ActionId::EnterEndPhase:
    d.turn.phase = Phase::End; return "End Phase.";
  ...
  case ActionId::DeckOut: return "deck out is checked at the mandatory draw.";
  case ActionId::CardEffectWin:
    SetResult(d, DuelResult::Player0Win, WinReason::CardEffectWin);
    return "duel decided by a card effect.";
  case ActionId::ViewGraveyard: case ActionId::PickUpGraveyard:
    return ViewGraveyard(d, args.targetPlayer >= 0 ? args.targetPlayer : me);
  case ActionId::ShuffleDeck:
    d.field.deckZones[me].shuffle(); return "deck shuffled.";
  case ActionId::CutDeck: return "deck cut.";
  ...
  case ActionId::CheckCannotChangePosition: case ActionId::CheckAlreadyChangedThisTurn:
  ...
  case ActionId::CheckTributeRequirement: case ActionId::RequireTribute:
  case ActionId::Level5to6Need1: case ActionId::Level7orHigherNeed2:
    return args.target ? "tributes required: " + std::to_string(TributesRequired(args.target)) + "."
                       : "tribute check needs a monster.";
  case ActionId::CheckFusionMaterials: case ActionId::CheckMaterialsInRequiredPlaces:
    return args.materials.size() >= 2 ? "materials present." : "need at least 2 materials.";
  case ActionId::CheckFlipEffect: return "flip effects trigger via the flip path.";
  case ActionId::CannotFlipSummonSameTurn:
    return CanFlipSummon(d, args.target) ? "flip summon legal." : "cannot flip summon now.";
  case ActionId::CannotPlayFaceUpDefense: return "face-up defense play is not allowed.";
  case ActionId::CheckHandSize: case ActionId::MoreThan6Cards:
  ...
  case ActionId::CannotEndTurn: case ActionId::HandLimitUnresolved:
    return CanEndTurn(d, me) ? "turn can end." : "hand limit unresolved.";
  case ActionId::ResolveEndPhaseEffects: case ActionId::AnnounceEndOfTurn:
    return EndTurn(d);
  case ActionId::ResetPerTurnState:
    ResetPerTurnState(d); return "per-turn state reset.";
  case ActionId::SwapPlayers:
    d.turnPlayer = 1 - d.turnPlayer; return "players swapped.";
  case ActionId::IncrementTurnNumber:
    ++d.turn.turnNumber; return "turn " + std::to_string(d.turn.turnNumber) + ".";
  case ActionId::FirstTurnSkips: case ActionId::StartingPlayerSkipDraw:
  ...
  case ActionId::ReduceLP0: return "LP depletion is checked by CheckWinConditions.";
  case ActionId::UnableToDraw: return "a player who must draw from an empty deck loses.";
  case ActionId::BattleStartStep: case ActionId::AnnounceEnteringBattlePhase:
    d.traceBattle(protocol::BattleStep::AttackerChosen); return "Battle Step started.";
  case ActionId::BattleStep: return "Battle Step in progress.";
  case ActionId::ProceedToDamageStep: return ResolveDamage(d);
  case ActionId::ReturnToBattleStep:
    d.traceBattle(protocol::BattleStep::Idle); return "returned to the Battle Step.";
  case ActionId::ReplayAfterFieldChange: case ActionId::MonsterRemovedBeforeDamageStep:
  case ActionId::NewMonsterPlayedBeforeDamageStep: case ActionId::ReSelectNewTarget:
  case ActionId::CanAttackSameMonster: case ActionId::CanAttackDifferentMonster:
  ...
  case ActionId::HasNotAttackedYet: case ActionId::CanAttackOnce:
  ...
  case ActionId::BuildChain:
    d.chain.step = protocol::ChainStep::Building; return "chain building.";
  case ActionId::NearestPreviousLink: case ActionId::ResolveInReverseOrder:
  case ActionId::ResolveLinkFirst: case ActionId::ResolveLinkLast:
    return "chain resolves last-link-first.";
  case ActionId::SendFusionMaterialsToGraveyard: case ActionId::SendTributedMonstersToGraveyard:
    return std::to_string(MoveMaterialsToGY(d.field, args.materials)) + " material(s) -> Graveyard.";
  case ActionId::TakeFusionMonsterFromExtraDeck: case ActionId::PlaceFusionMonsterInExtraMonsterZone:
  ...
  case ActionId::HaveRitualSpellInHand: return "the ritual spell must be in your hand.";
  case ActionId::HaveMatchingRitualMonster: case ActionId::HaveRequiredTribute:
  case ActionId::TributeForRitualSummon: case ActionId::ActivateRitualSpellCard:
  ...
  case ActionId::SynchroSummon: case ActionId::DeclaresSynchroSummon:
  case ActionId::NeedOneTuner: case ActionId::CheckTunerMonster:
  case ActionId::CheckNonTunerMonsters: case ActionId::SumLevelsMustEqualSynchroLevel:
  case ActionId::SendSynchroMaterialsToGraveyard: case ActionId::TakeSynchroMonsterFromExtraDeck:
    return SynchroGate(d);
  case ActionId::XyzSummon: case ActionId::ChooseXyzMonsterFromExtraDeck:
  case ActionId::CheckXyzMaterials: case ActionId::CheckXyzMaterialsFaceUp:
  case ActionId::DeclareXyzSummoning: case ActionId::StackXyzMaterials:
  case ActionId::PlaceXyzMonsterOnTop: case ActionId::DetachXyzMaterial:
  case ActionId::SendXyzMaterialToGraveyard:
    return XyzGate(d);
  case ActionId::PendulumSummon: case ActionId::ActivatePendulumMonsterAsSpell:
  case ActionId::ActivateInLeftmostZone: case ActionId::ActivateInRightmostZone:
  case ActionId::ZoneBecomesPendulumZone: case ActionId::HaveOnePendulumInEachZone:
  case ActionId::DeclarePendulumSummoning: case ActionId::CheckPendulumScales:
  case ActionId::CheckMonstersInHand: case ActionId::CheckMonstersInExtraDeck:
  case ActionId::LevelsMustBeBetweenScales: case ActionId::PendulumMonsterGYToExtraDeck:
  case ActionId::PlaceInExtraMonsterZoneOrPointedZone:
    return PendulumGate(d);
  case ActionId::LinkSummon: case ActionId::CheckLinkMaterials:
  case ActionId::CheckLinkRating: case ActionId::SendMaterialsToGraveyard:
  case ActionId::MatchMaterialRequirements: case ActionId::PlaceInPointedZone:
  case ActionId::LinkMaterialCanBeLinkMonster: case ActionId::CountLinkMonsterAs1OrLinkRating:
  case ActionId::LinkArrowPointsToZone: case ActionId::MonsterIsLinked:
  case ActionId::CoLinked:
    return LinkGate(d);
  case ActionId::PlaceInExtraMonsterZone:
    return "extra monster zone placement (via fusion/special summon).";
  case ActionId::None: return "idle.";
  }
  auto r = "action id " + std::to_string(static_cast<int>(id)) + " not handled.";
  return r.empty() ? "(resolved.)" : r;
}

} // namespace openjooy::engine::action
