...
inline ActionResult Perform(Duel &d, ActionId id, const ActionArgs &args = {}) {
    ...
    switch (id) {
        ...
        case ActionId::Move_SearchToHand:
            return SearchDeck(d, [](const Card &) { return true; })
        ...
        case ActionId::Move_Excavate: {
            ...
            return ActionResult::Ok("excavated " + std::to_string(cards.size()) +
                                    " card(s).");
        }
        case ActionId::Summon_Token:
            // Token stats come from the caller (0/0 was a silent stub).
            return SummonToken(d, "Token", args.atk, args.def)
                       ? ActionResult::Ok("token summoned.")
                       : ActionResult::Fail("no free monster zone.");
        ...
        case ActionId::Pos_ChangeAToDef:
            return PosChange(d.field, args.target, zone::Orientation::Horizontal)
                       ? ActionResult::Ok("switched to Defense Position.")
                       : ActionResult::Fail(
        ...
        case ActionId::Pos_ChangeDefToAtk:
            return PosChange(d.field, args.target, zone::Orientation::Vertical)
                       ? ActionResult::Ok("switched to Attack Position.")
                       : ActionResult::Fail(
                             "position change: target must be a face-up monster.");
        case ActionId::Pos_Flip:
            return PosFlip(d.field, args.target)
                       ? ActionResult::Ok("flipped face-up (Flip effect may trigger).")
                       : ActionResult::Fail("flip: target is not a set monster.");
        ...
        case ActionId::Counter_Place:
            ...
            return ActionResult::Ok("counters placed.");
        case ActionId::Counter_Remove:
            return ActionResult::Ok(
                "removed " +
                std::to_string(RemoveCounterD(d, args.target, "counter", args.n)) +
                " counter(s).");
        ...
        case ActionId::SkipDraw:
            return ActionResult::Ok("draw skipped.");
        ...
        case ActionId::SkipBattlePhase:
            return d.turn.phase == Phase::Main1
                       ? ActionResult::Ok("proceed to Main Phase 2 to skip battle.")
                       : ActionResult::Fail("cannot skip now.");
        ...
        case ActionId::ChooseAttackOrDefensePosition:
            return ActionResult::Ok("choose: face-up ATK or face-down DEF.");
        ...
        case ActionId::SetTrapCard: {
            ...
            if (SeatSpellTrap(d.field, c))
                return ActionResult::Ok(std::string(c ? c->name : "card") +
                                            " set face-down.",
                                        id);
        ...
        case ActionId::CanChooseNotToAttack:
            return ActionResult::Ok("you may choose not to attack.");
        case ActionId::CancelAttack:
            CancelAttack(d);
            return ActionResult::Ok("attack cancelled.");
        ...
        case ActionId::SelectAndDiscard: {
            ...
            if (moveTo(d.field, args.target, d.field.graveyardZones[me]))
                return ActionResult::Ok("discarded.");
        ...
        case ActionId::CheckWinConditions:
            return CheckWinConditions(d) == DuelResult::Ongoing
                       ? ActionResult::Ok("duel ongoing.")
                       : ActionResult::Ok("duel decided.");
        case ActionId::DeckOut:
            return ActionResult::Ok("deck out is checked at the mandatory draw.");
        case ActionId::CardEffectWin:
            // The winner is the ACTIVATOR, not hardcoded player 0.
            SetResult(d, me == 0 ? DuelResult::Player0Win : DuelResult::Player1Win,
                      WinReason::CardEffectWin);
        ...
        case ActionId::ShuffleDeck:
            ...
            return ActionResult::Ok("deck shuffled.");
        case ActionId::CutDeck:
            return ActionResult::Ok("deck cut.");
        case ActionId::CheckDirectAttackLegal:
            return CanDirectAttack(d, args.target)
                       ? ActionResult::Ok("direct attack legal.")
                       : ActionResult::Fail("opponent controls monsters.");
        case ActionId::CheckOpponentFieldEmpty:
            return OpponentFieldEmpty(d, me)
                       ? ActionResult::Ok("opponent field empty.")
                       : ActionResult::Fail("opponent controls monsters.");
        case ActionId::CheckReplay:
            return ConfirmAttack(d) ? ActionResult::Ok("replay check passed.")
                                    : ActionResult::Fail("replay! attack cancelled.");
        case ActionId::CheckCannotChangePosition:
        case ActionId::CheckAlreadyChangedThisTurn:
            return CanChangePosition(d, args.target)
                       ? ActionResult::Ok("position change legal.")
                       : ActionResult::Fail("position change illegal now.");
        case ActionId::CheckMonsterPlayedThisTurn:
            return HasPlacedMonsterThisTurn(d, me)
                       ? ActionResult::Ok("a monster was played this turn.")
                       : ActionResult::Ok("none played.");
        ...
        case ActionId::Level7orHigherNeed2:
            return args.target
                       ? ActionResult::Ok("tributes required: " +
                                          std::to_string(TributesRequired(args.target)) +
                                          ".")
                       : ActionResult::Fail("tribute check needs a monster.");
        case ActionId::CheckFusionMaterials:
        case ActionId::CheckMaterialsInRequiredPlaces:
            return args.materials.size() >= 2
                       ? ActionResult::Ok("materials present.")
                       : ActionResult::Fail("need at least 2 materials.");
        case ActionId::CheckFlipEffect:
            return ActionResult::Ok("flip effects trigger via the flip path.");
        case ActionId::CannotFlipSummonSameTurn:
            return CanFlipSummon(d, args.target)
                       ? ActionResult::Ok("flip summon legal.")
                       : ActionResult::Fail("cannot flip summon now.");
        case ActionId::CannotPlayFaceUpDefense:
            return ActionResult::Fail("face-up defense play is not allowed.");
        case ActionId::CheckHandSize:
        case ActionId::MoreThan6Cards:
            return ActionResult::Ok(
                "hand size: " + std::to_string(HandSize(d, me)) +
                (OverHandLimit(d, me) ? " (over limit)." : " (within limit)."));
        case ActionId::DiscardUntilHas6:
            return ActionResult::Ok("discarded " +
                                    std::to_string(DiscardToHandLimit(d, me)) +
                                    " card(s).");
        case ActionId::CannotEndTurn:
        case ActionId::HandLimitUnresolved:
            return CanEndTurn(d, me) ? ActionResult::Ok("turn can end.")
                                     : ActionResult::Fail("hand limit unresolved.");
        case ActionId::ResolveEndPhaseEffects:
        case ActionId::AnnounceEndOfTurn:
            return EndTurn(d);
        case ActionId::ResetPerTurnState:
            ResetPerTurnState(d);
            return ActionResult::Ok("per-turn state reset.");
        case ActionId::SwapPlayers:
            d.turnPlayer = 1 - d.turnPlayer;
            return ActionResult::Ok("players swapped.");
        case ActionId::IncrementTurnNumber:
            ++d.turn.turnNumber;
            return ActionResult::Ok("turn " + std::to_string(d.turn.turnNumber) + ".");
        case ActionId::FirstTurnSkips:
        case ActionId::StartingPlayerSkipDraw:
            return ActionResult::Ok("starting player skips the first draw.");
        case ActionId::StartingPlayerSkipBattle:
            return ActionResult::Ok("starting player skips the first Battle Phase.");
        case ActionId::Win:
            return d.result == DuelResult::Ongoing
                       ? ActionResult::Ok("no winner yet.")
                       : ActionResult::Ok("duel decided.");
        case ActionId::ReduceLP0:
            return ActionResult::Ok("LP depletion is checked by CheckWinConditions.");
        case ActionId::UnableToDraw:
            return ActionResult::Ok("a player who must draw from an empty deck loses.");
        case ActionId::BattleStartStep:
        case ActionId::AnnounceEnteringBattlePhase:
            d.traceBattle(protocol::BattleStep::AttackerChosen);
            return ActionResult::Ok("Battle Step started.");
        case ActionId::BattleStep:
            return ActionResult::Ok("Battle Step in progress.");
        case ActionId::ProceedToDamageStep:
            return ResolveDamage(d);
        case ActionId::ReturnToBattleStep:
            d.traceBattle(protocol::BattleStep::Idle);
            return ActionResult::Ok("returned to the Battle Step.");
        ...
        case ActionId::FirstMonsterStillConsideredAttacked:
            return ConfirmAttack(d)
                       ? ActionResult::Ok("replay check passed — attack stands.")
                       : ActionResult::Fail("replay! re-declare.");
        case ActionId::CannotAttackAgain:
            return HasNotAttacked(d, args.target)
                       ? ActionResult::Ok("has not attacked yet.")
                       : ActionResult::Fail("cannot attack again this turn.");
        case ActionId::HasNotAttackedYet:
        case ActionId::CanAttackOnce:
        case ActionId::FaceUpAttackPosition:
            return CanAttack(d, args.target)
                       ? ActionResult::Ok("attack legal.")
                       : ActionResult::Fail("attack not legal for that monster.");
        case ActionId::CanAttackMultipleMonsters:
            return ActionResult::Fail("multiple attacks are not legal in the classic format.");
        case ActionId::CanCancelAttack:
            return CanCancelAttack(d) ? ActionResult::Ok("attack can be cancelled.")
                                      : ActionResult::Fail("nothing to cancel.");
        case ActionId::CannotSkipIfMonsterOnField:
            return OpponentFieldEmpty(d, me)
                       ? ActionResult::Ok("skip possible.")
                       : ActionResult::Fail("cannot skip: monsters on the field.");
        case ActionId::FirstPlayerCannotBattle:
            return d.turn.skipBattle
                       ? ActionResult::Ok("first turn: no Battle Phase.")
                       : ActionResult::Ok("battle allowed.");
        case ActionId::BuildChain:
            d.chain.step = protocol::ChainStep::Building;
            return ActionResult::Ok("chain building.");
        ...
        case ActionId::ResolveLinkLast:
            return ActionResult::Ok("chain resolves last-link-first.");
        case ActionId::SendFusionMaterialsToGraveyard:
        case ActionId::SendTributedMonstersToGraveyard:
            return ActionResult::Ok(
                std::to_string(MoveMaterialsToGY(d.field, args.materials)) +
        ...
        case ActionId::PlaceFusionCardInSpellTrapZone:
            return ActionResult::Ok("the fusion spell resolves from the spell/trap zone.");
        case ActionId::PlaceFusionSummoningCardInGraveyard:
            return ActionResult::Ok("the fusion spell hits the Graveyard after resolving.");
        case ActionId::HaveRitualSpellInHand:
            return ActionResult::Ok("the ritual spell must be in your hand.");
        ...
        case ActionId::PlaceRitualSpellCardInGraveyard:
            return ActionResult::Ok("the ritual spell hits the Graveyard after resolving.");
        ...
        case ActionId::PlaceInExtraMonsterZone:
            return ActionResult::Ok("extra monster zone placement (via fusion/special summon).");
        case ActionId::None:
            return ActionResult::Ok("idle.");
        default:
            break;  // any id not realized here falls through to the verdict below
    }
    return ActionResult::Fail("action id " +
