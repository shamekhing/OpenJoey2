...
inline int SearchDeck(Duel &d, const std::function<bool(const Card &)> &pred) { return MoveSearchToHand(d.field, d.turnPlayer, pred); }
inline std::vector<Card *> Excavate(Duel &d, int n) { return MoveExcavate(d.field, d.turnPlayer, n); }
inline ActionResult ResolveStandby(Duel &d) {
    ...
    if (d.config.chainResponseWindow) return ActionResult::Ok("standby triggers wait on the chain.");
