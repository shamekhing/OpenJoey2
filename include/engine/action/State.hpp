...
inline bool CanAttack(const Duel &d, Card *c) {
    ...
    return BattlePhaseOpen(d) && mz &&
           c->state.controller == d.turnPlayer && mz->isVisible() &&
           mz->position() == zone::Orientation::Vertical &&
           !d.turnState.attacked.count(c);
}
inline bool CanDirectAttack(const Duel &d, Card *c) {
    return CanAttack(d, c) && OpponentFieldEmpty(d, c->state.controller);
}
inline bool AttackOpen(const Duel &d) {
    return d.turnState.pending.attacker != nullptr;
}
inline bool CanCancelAttack(const Duel &d) { return AttackOpen(d); }
inline bool HasNotAttacked(const Duel &d, Card *c) {
...
inline bool CanFlipSummon(const Duel &d, Card *c) {
    ...
    return d.result == DuelResult::Ongoing &&
           (d.turn.phase == Phase::Main1 || d.turn.phase == Phase::Main2) &&
           mz && c->state.controller == d.turnPlayer && !mz->isVisible() &&
           !c->state.setThisTurn;
}
inline bool CanChangePosition(const Duel &d, Card *c) {
    ...
    return d.result == DuelResult::Ongoing &&
           (d.turn.phase == Phase::Main1 || d.turn.phase == Phase::Main2) &&
           mz && c->state.controller == d.turnPlayer && mz->isVisible() &&
           !c->state.placedThisTurn && !c->state.setThisTurn &&
