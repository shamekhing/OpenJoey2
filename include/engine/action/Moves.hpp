...
inline bool moveTo(Field &f, Card *c, zone::IZone &dest) {
  ...
  Card *r = z->remove(c);
  if (!r) return false;
  if (!dest.put(r)) { z->put(r); return false; }
  return true;
}

namespace detail { // UI-facing relocation (resolved spells/traps -> Graveyard)
inline bool moveCard(Field &f, Card *c, zone::IZone &dest) {
  return moveTo(f, c, dest);
}
} // namespace detail
...
inline bool eraseToken(Field &f, Card *c) {
  ...
  for (auto it = tk.begin(); it != tk.end(); ++it)
    if (it->get() == c) { tk.erase(it); return true; }
  return false;
}
...
inline bool sweepOffField(Field &f, Card *c) {
  ...
  while (!c->state.equippedCards.empty())
    if (Card *e = c->state.equippedCards.back())
      MoveDestroyToGY(f, e);
    else
      c->state.equippedCards.pop_back();
...
inline int MoveMaterialsToGY(Field &f, const std::vector<Card *> &mats) {
  int n = 0;
  for (Card *m : mats) if (MoveDestroyToGY(f, m)) ++n;
  return n;
}
inline int MoveSearchToHand(Field &f, int player,
                            const std::function<bool(const Card &)> &pred) {
...
inline int MoveDestroyMass(Field &f, openjoey::TargetScope scope, int me) {
  int n = 0;
  auto row = [&](int p) { for (auto &mz : f.monsterZones[p]) if (Card *c = mz.peek()) { MoveDestroyToGY(f, c); ++n; } };
  auto st = [&] { for (int p = 0; p < 2; ++p) for (auto &z : f.spellTrapZones[p]) if (Card *c = z.peek()) { MoveDestroyToGY(f, c); ++n; } };
  switch (scope) {
  case openjoey::TargetScope::OppMonsters: row(1 - me); break;
  case openjoey::TargetScope::AllMonsters: row(0); row(1); break;
  case openjoey::TargetScope::AllSpellsTraps: st(); break;
  case openjoey::TargetScope::OppAttackPos:
    for (auto &mz : f.monsterZones[1 - me])
      if (Card *c = mz.peek())
        if (mz.position() == zone::Orientation::Vertical) { MoveDestroyToGY(f, c); ++n; }
    break;
  default: break;
  }
  return n;
...
inline bool SummonToMMZ(Field &f, Card *c, int player, SummonPose pose) {
  ...
  if (slot < 0) { z->put(c); return false; }
  auto &mz = f.monsterZones[player][slot];
  if (!mz.put(c)) { z->put(c); return false; }
  mz.changeOrientation(pose == SummonPose::Atk ? zone::Orientation::Vertical
                                               : zone::Orientation::Horizontal);
  mz.changeVisibility(pose == SummonPose::DefDown ? zone::Visibility::Limited
...
inline bool SummonToMMZ(Field &f, Card *c, int player, bool faceDown) {
  return SummonToMMZ(f, c, player,
                     faceDown ? SummonPose::DefDown : SummonPose::Atk);
}
inline bool SummonFromZone(Field &f, Card *c, int player, bool toEMZ,
                           SummonPose pose) {
  if (!c) return false;
  auto [z, p] = f.findCard(c);
  if (!z || !z->remove(c)) return false;
  if (toEMZ) {
    ...
    if (zi < 0) { z->put(c); return false; }
    auto &emz = f.extraMonsterZones[zi];
    if (!emz.put(c)) { z->put(c); return false; }
...
inline bool SummonFromZone(Field &f, Card *c, int player, bool toEMZ,
                           bool faceDown = false) {
  return SummonFromZone(f, c, player, toEMZ,
                        faceDown ? SummonPose::DefDown : SummonPose::Atk);
}
inline bool PlaceFusion(Field &f, Card *extra) {
  ...
  auto &emz = f.extraMonsterZones[zi];
  if (!emz.put(extra)) { z->put(extra); return false; }
  emz.changeOrientation(zone::Orientation::Vertical);
  emz.changeVisibility(zone::Visibility::Visible);
  return true;
}
...
inline bool PosFlip(Field &f, Card *c) {
  if (!c) return false;
  auto [z, p] = f.findCard(c);
  if (z) if (auto *mz = dynamic_cast<zone::Zone_Monster *>(z)) return mz->flip();
  return false;
}
inline bool PosChange(Field &f, Card *c, zone::Orientation to) {
  if (!c) return false;
  auto [z, p] = f.findCard(c);
  if (z) if (auto *mz = dynamic_cast<zone::Zone_Monster *>(z)) return mz->changeOrientation(to);
  return false;
}
...
inline bool SeatSpellTrap(Field &f, Card *c) {
  ...
  if (!stz.put(c)) { z->put(c); return false; }
...
inline Card *SummonTokenMat(Field &f, const std::string &name, int atk, int def, int player) {
  ...
  tok->name = name; tok->atk = atk; tok->def = def;
