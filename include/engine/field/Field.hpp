...
public:
  ...
  IZone* zoneAt(int player, ZoneType t, int i) {
    if (player < 0 || player >= PLAYERS)
      return nullptr;
    switch (t) {
    case ZoneType::Monster:      return (i >= 0 && i < MONSTER_ZONES)
                                        ? &monsterZones[player][i] : nullptr;
    case ZoneType::SpellTrap:    return (i >= 0 && i < ST_ZONES)
                                        ? &spellTrapZones[player][i] : nullptr;
    case ZoneType::Field:        return &fieldZones[player];
    case ZoneType::ExtraMonster: return (i >= 0 && i < EMZ_COUNT)
                                        ? &extraMonsterZones[i] : nullptr;
