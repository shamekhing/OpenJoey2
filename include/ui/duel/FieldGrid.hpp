...
class FieldGrid {
   public:
    ...
    int ownerOf(IZone* z, Field& field) const {
        if (!z) return -1;
        for (int p = 0; p < 2; ++p) {
            for (auto& z2 : field.monsterZones[p])
                if (z == &z2) return p;
            for (auto& z2 : field.spellTrapZones[p])
                if (z == &z2) return p;
