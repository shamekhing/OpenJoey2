...
struct DeckPanels {
    ...
    void drawPool(const std::vector<const openjoey::cards::Card*>& fp, int x, int y, int w, int h) const {
        std::string badge = std::string(sortModeLabel(sortMode)) + "  [" + typeFilterLabel(typeFilter) + "]  " + std::to_string(fp.size()) + " cards";
        Panel::Draw("Card Pool", badge.empty() ? nullptr : badge.c_str(), x, y, w, h, focusPool);

        int searchY = y + SEARCH_BAR_Y_OFFSET;
        searchInput.Draw(x + THUMBNAIL_PAD, searchY, w - THUMBNAIL_PAD * 2, SEARCH_BAR_HEIGHT(h));

        int listY = searchY + LIST_Y_OFFSET;
        CardList::Draw(fp, ctx.imageCache, x, listY, w, y + h - listY, poolNav.cursor, focusPool, DeckLimits::kMaxCopies, [&](uint32_t id) { return countCopies(deck, id); });
    }

    // ── Preview (art, type line, copy count, wrapped description) ───────────
    void drawPreview(const std::vector<const openjoey::cards::Card*>& fp, int x, int y, int w, int h) const {
        ...
        if (focusPool && !fp.empty() && poolNav.cursor < (int)fp.size()) card = fp[poolNav.cursor];
        else if (!focusPool && !deck.empty() && deckNav.cursor < (int)deck.size()) card = &deck[deckNav.cursor];
        if (!card) return;
        ...
        if (artY + artH > y + h - MAIN_PAD_BOTTOM / 2) artH = y + h - MAIN_PAD_BOTTOM / 2 - artY;
        ...
        if (tex && tex->id != 0) {
            ...
            DrawRectangle(artX, artY, artW, artH, Color{col.r, col.g, col.b, CARD_PREVIEW_ALPHA});
            int nameW = MeasureText(card->name.c_str(), FONT_CARD_NAME);
            DrawText(card->name.c_str(), artX + (artW - nameW) / 2, artY + artH / 2 - FONT_CARD_NAME / 2, FONT_CARD_NAME, WHITE);
        }
        DrawRectangleLines(artX, artY, artW, artH, col);
        DrawText(card->cardTypeTag().c_str(), artX + THUMBNAIL_PAD, artY + THUMBNAIL_PAD, FONT_CARD_STAT, col);
        ...
        Color cpCol = (copies >= DeckLimits::kMaxCopies) ? RED : (copies > 0) ? GREEN : GRAY;
        DrawText(TextFormat("In deck: %d / %d", copies, DeckLimits::kMaxCopies), x + PREVIEW_PAD_X, infoY, FONT_CARD_STAT, cpCol);
        ...
        while (cur < (int)desc.size() && infoY < y + h - THUMBNAIL_PAD) {
            ...
            DrawText(desc.substr(cur, end - cur).c_str(), x + PREVIEW_PAD_X, infoY, FONT_CARD_STAT, COLOR_DESC_TEXT);
            infoY += lineH;
            cur = end;
    ...
    void drawDeck(int x, int y, int w, int h) const {
        ...
        DeckStats::Draw(deck, DeckLimits::kMinDeckSize, x + PREVIEW_PAD_X, y + SEARCH_BAR_Y_OFFSET, w - PREVIEW_ART_SIDE_PAD);
        ...
        if (deckGridView) {
            CardGrid::Draw(deckPtrs, ctx.imageCache, x, listY, w, listH, deckNav.cursor, !focusPool);
        } else {
            CardList::Draw(deckPtrs, ctx.imageCache, x, listY, w, listH, deckNav.cursor, !focusPool, DeckLimits::kMaxCopies, [&](uint32_t id) { return countCopies(deck, id); });
        }
    }
};
